...
**SMB.** This is the one genuinely new dependency risk. `hirochachacha/go-smb2` provides a `Krb5Initiator` GSSAPI initiator, which is exactly what's needed — but the original is lightly maintained. Two active forks exist: [`jfjallid/go-smb`](https://github.com/jfjallid/go-smb) (SMB3, active mid-2026) and [`CloudSoda/go-smb2`](https://github.com/CloudSoda/go-smb2).

> **Phase 0 spike, mandatory.** Pick the SMB library by testing against a real DC with **SMB signing required** (the default for SYSVOL/NETLOGON) and SMB3 encryption enabled, authenticating by **Kerberos, not NTLM** — NTLM is disabled outright in many hardened domains. If none of the three work, the fallback is a `cifs` mount in a privileged container, which is a materially worse product. Do not commit to the architecture until this is proven.
