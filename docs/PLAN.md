...
**Ordering — get this right; it is the reverse of intuition.** The **last** entry in the `gPLink` string has Link Order 1 and the **highest** precedence. Reading the string left to right, precedence *decreases*; to produce links in precedence order (highest first), reverse the string. This is established by the O-03 oracle (`testdata/oracle/gplink/`), not by reading the spec: case `11_three_abc` stores `[c][b][a]` and GPMC reports a=Order 1, b=2, c=3; the raw-written case `29_dup_0_2` stores `[a;0][a;2]` and GPMC reports the last entry (`;2`) as Order 1. `New-GPLink` prepends each new link to the front of the string at the lowest precedence, so the first-created link ends up last in the string with Link Order 1. The `$links.count - $i` formula from the widely-copied PFE script agrees with this; the earlier claim in this plan that it was "the opposite" was itself inverted, which is exactly why O-03 captures GPMC directly rather than trusting any prose.

> Do not resolve this from spec prose or third-party scripts: O-03 captured GPMC's actual link ordering into `testdata/oracle/gplink/`, and D-03 is tested against those fixtures. An implementation that disagrees with the fixtures is wrong, whatever any spec text, blog post, or PFE script says. This gets a dedicated test.
